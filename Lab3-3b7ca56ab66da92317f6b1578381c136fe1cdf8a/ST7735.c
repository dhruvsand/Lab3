...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
  int16_t skipC = 0,copy;                      // non-zero if columns need to be skipped due to clipping
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
		copy =  ST7735_SwapColor(image[i]) ; //this is the change Dhruv made also changed image[i] to copy for next two lines
			// send the top 8 bits
//      writedata((uint8_t)(image[i] >> 8));
			writedata((uint8_t)(copy >> 8));
                                        // send the bottom 8 bits
//      writedata((uint8_t)image[i]);
			writedata((uint8_t)copy);
...
//	while((x1!=x2)||(y1!=y2)){
//		ST7735_DrawPixel(int16_t x1, int16_t y1, uint16_t color);
//		
//	}
...
void ST7735_DrawCue(float xcen, float ycen,float xbond, float ybond, int16_t textColor){
  ...
  float xtemp,ytemp;
	xtemp=xcen;
	ytemp=ycen;
	ST7735_DrawPixel(xtemp, ytemp, textColor);
	if(xcen<xbond)
		xtemp++;
	else if (xcen>xbond)
		xtemp--;
	...
	ST7735_DrawPixel(xbond, ybond, textColor);
	return;
}
